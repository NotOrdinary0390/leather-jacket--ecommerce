<template>
  <div class="mx-4 max-[340px]:mx-1">
    <p class="text-sm tracking-wider mb-3 ml-4">My Account</p>

    <form @submit.prevent="handleLogin">
      <div class="form-container font-primary">
        <label class="text-xs tracking-wider mb-2" for="email">Email</label>
        <input type="email" class="input-form" v-model="loginData.email" />
    ...
    <form @submit.prevent="handleResetPassword()">
      <div v-if="showRecovery" class="form-container">
        <p class="text-sm tracking-wider mb-3">Retrieve Password</p>
        <label class="text-xs tracking-wider mb-2 font-primary" for="email"
          >Email</label
        >
        <input
          type="email"
        ...
        <button type="submit" class="btn-form font-primary">
          RESET PASSWORD
        </button>
      </div>
    </form>
